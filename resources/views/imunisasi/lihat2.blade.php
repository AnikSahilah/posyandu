            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($balita->imunisasi as $index => $imunisasi)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $index + 1 }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    {{ $imunisasi->petugas->nama_petugas ?? '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ \Carbon\Carbon::parse($imunisasi->tanggal_imunisasi)->translatedFormat('d F Y') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600 font-medium">
                                    {{ $imunisasi->jenis->jenis_imunisasi ?? '-' }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    {{ $imunisasi->jenis->keterangan ?? '-' }}
                                </td>
                            </tr>
                        @empty
                            <tr>
